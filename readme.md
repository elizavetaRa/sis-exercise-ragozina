...
Elizaveta: Instructions to run the app


## Task 1: Celery for harversing inspirehep api data
...
3. In another terminal copy the celery docker-container id by running 
    ```bash
    docker ps
        ```
...
4. Run celery worker 
    ```bash
sudo docker exec -it f <celery-docker-container-id> celery -A sis_exercise worker --loglevel=info
    ```
5. Run celery beat

    ```bash
sudo docker exec -it <celery-docker-container-id> celery -A sis_exercise beat --loglevel=info
    ```
